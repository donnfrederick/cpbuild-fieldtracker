...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    context.log('HTTP trigger function processed a request to list all the work hour submissions by worker and project scope.');
    ...
    const query = `
        ...
        JOIN field_tracker.work_pay_types wpt
            ON whst.pay_type_id = wpt.id
        ...
        JOIN field_tracker.unit_tasks ut
            ON whs.task_id = ut.id
        WHERE whs.worker_id = @workerId
        AND whs.project_by_scope_id = @projectByScopeId
        AND whs.deleted_at IS NULL
        ORDER BY whs.id DESC;
    `;
        ...
        if (result.recordset.length > 0) {
            ...
            context.res = {
                body: {
                    ...
                    message: 'Successfully retrieved work hour submissions.'
                }
            };
        } else {
            context.res = {
