...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    const query = `
            ...
            LEFT JOIN field_tracker.unit_tasks root_ut
            ON ut.root_main_task_id = root_ut.id
            LEFT JOIN field_tracker.task_types rtt
                ON root_ut.task_type_id = rtt.id
            LEFT JOIN field_tracker.task_status_types rst
                ON root_ut.status_id = rst.id
            WHERE 
                upbs.phase_name = 'Clear Inspection'
            AND tupbs.phase_name = 'Clear Inspection'
            AND ubs.status_id IN (3, 4)
            AND (
                (tt.type_name = 'Punch Work' AND ut.status_id = 4 AND rtt.type_name = 'Main')
                OR
                (tt.type_name != 'Main' AND tt.type_name != 'Punch Work' AND ut.status_id IN (2, 3))
            )
