...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    context.log('HTTP trigger function processed a request to list all the unit level work hour submission types.');

    const pool = await initializePool(databaseIdentifier, sqlConfig);
    const phaseByScopeId = context.bindingData.phaseByScopeId;
    ...
    let query = `
        ...
        AND tlt.id = 1
        AND (
        	whst.phase_id IS NULL
            OR whst.phase_id = @phaseByScopeId
        );
    `;
    ...
    try {
        ...
        const result = await request
            .input('phaseByScopeId', sql.Int, phaseByScopeId)
            .query(query);
