...
interface NewInstallTeamInput {
    teamName: string;
    statusId: number;
    createdBy: number;
}

async function validateNewTeam(data: NewInstallTeamInput): Promise<string | null> {
...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    const installTeamData = req.body as NewInstallTeamInput;
    const validationError = await validateNewTeam(installTeamData);
    ...
    const insertQuery = `
        INSERT INTO field_tracker.install_teams (team_name, status_id, created_by)
        VALUES (@teamName, @statusId, @createdBy);
    `;
    ...
    try {
        ...
        const insertResult = await request
            ...
            .input('createdBy', sql.Int, installTeamData.createdBy)
            .query(insertQuery);
