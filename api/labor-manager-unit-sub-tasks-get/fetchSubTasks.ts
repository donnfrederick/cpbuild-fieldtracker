...
export default async function fetchSubTasks(unitByScopeId: number, context: Context) {
    context.log('HTTP trigger function processed a request to fetch all unit sub tasks.');
    try {
        ...
        .input('unitByScopeId', sql.Int, unitByScopeId)
            .query(`
                ...
                LEFT JOIN dbo.users su
                    ON sw.user_id = su.id
                WHERE ut.task_type_id IN (2, 3, 4)
                AND ut.unit_by_scope_id = @unitByScopeId
            `);
