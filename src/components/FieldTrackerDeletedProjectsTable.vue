<script setup lang="ts">
  ...
  const deletedProjectsList = computed(() => toolStore.fieldTrackerDeletedProjectsData);
  const showTable = computed(() => toolStore.activeFieldTrackerTab === 'deleted');
  ...
  const setProjectDataByStatus = async (statusType: string) => {
    ...
    if (projectDataResponse.error) {
      ...
      if (tabulator.value) {
        tabulator.value.setData(deletedProjectsList.value);
      }
    }
  };
  ...
  const initializeTabulator = () => {
    ...
    tabulator.value = new Tabulator(table.value, {
      ...
      data: deletedProjectsList.value,
      reactiveData: true,
      layout: 'fitDataTable',
      clipboard: true,
  ...
  const reactivateProject = async (ftProjectId: number) => {
    ...
    localStorage.setItem('activeFieldTrackerTab', 'deleted');
    ...
    onRendered(() => {
      ...
      if (authStore.hasAdminRole || authStore.hasControlsManagerRole) {
        ...
        cell
          ...
          ?.addEventListener('click', (e) => {
            e.stopPropagation();
            completeProject(projectData.rootProjectId); // same as above. rootProjectId and ftProjectId need to be swapped
          });
      }
    });
  ...
  onMounted(async () => {
    ...
    if (toolStore.fieldTrackerCompletedProjectsData === null) {
      await setProjectDataByStatus('deleted');
    } else {
      await toolStore.getFieldTrackerProjectsData('deleted');
    }

    // Iitialize Tubulator after data is fetched
    if (deletedProjectsList.value) {
      initializeTabulator();
    } else {
      hasApiError.value = true;
      apiErrorMessage.value =
        'There was an issue loading deleted projects. Please refresh the page to try again. If the does not work, try closing the browser tab and navigating back to the tool.';
    }
    isLoading.value = false;
  });
...
<template>
  <div>
    ...
    <div v-show="showTable" ref="table" class="deleted-table-container"></div>
  </div>
</template>
